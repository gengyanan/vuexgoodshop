    ...
    <div>
      ...
      <div class="params-colors">
        <ul class="colors-list">
          <li v-for="(sku,index) in item.sku_info" @click="tableIndex(index)">
            <a href="javascript:;"
               :class="{'active':index===itemIndex}"
               :title="sku.spec_json.show_name"><img
              :src="'http://img01.smartisanos.cn/'+sku.spec_json.image+'/20X20.jpg'"></a></li>
        </ul>
      </div>
      <div class="item-btns clearfix">
        ...
        class="item-blue-btn"
        @click="addCarPanelHandle(item.sku_info[itemIndex])">加入购物车 </span>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      addCarPanelHandle(data) {
        let dataItem = {info: data, count: 1}
        this.$store.commit('addCalPanelDate', dataItem)
      },
    }
  }
</script>
